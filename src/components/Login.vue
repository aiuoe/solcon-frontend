...
export default class Login extends Vue {
	...
	async created()
	{
		...
		setInterval(() =>
		{
			this.radius = Math.floor(Math.random() * (200 - 300)) + 200
		}, 1000)
	}
			...
			.then(res => { 
				...
				if (this.$store.state.company == undefined)
				{
					...
					.then((res: any) => {
						...
						.catch((err: any) => console.log('Error login:', err))
				}
			})
			.catch(error => { 
				this.error = true 
